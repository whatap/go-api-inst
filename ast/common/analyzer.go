...
func detectFramework(importPath string) *Framework {
	...
	for _, fw := range frameworks {
		if strings.HasPrefix(importPath, fw.prefix) {
			...
			if remaining == "" || isVersionPathSuffix(remaining) {
				return &Framework{
					Name:       fw.name,
...
// isVersionPathSuffix checks if the string is a version path suffix (/v2, /v3, etc.)
func isVersionPathSuffix(s string) bool {
	if len(s) < 2 {
		return false
	}
	if s[0] != '/' {
