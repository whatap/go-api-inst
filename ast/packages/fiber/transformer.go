...
func (t *Transformer) Inject(file *dst.File) (bool, error) {
	...
	if t.transformed {
		common.AddImport(file, `"github.com/whatap/go-api/instrumentation/github.com/gofiber/fiber/v2/whatapfiber"`)
	}
...
// Example: app.Use(whatapfiber.Middleware())
...
func isWhatapMiddlewareCall(stmt dst.Stmt) bool {
	...
	// Check if argument is whatapfiber.Middleware()
